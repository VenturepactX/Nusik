...
<?php if(Yii::app()->user->id==$model->users->id){?><a href="#ChangeCover" data-toggle="modal" class="pull-right btn btn-primary mgtop10" > Change Cover </a><?php }?>
			...
			<section class="row-fluid">
				...
				<!-- Social Buttons-->
					<div class="span3 offset3 pull-right mgtop10">
						...
						<div class="thumbnails span3 offset5"><a href="<?php echo 'https://www.twitter.com/'. $model->artistsProfiles[0]->tw_link;?>" target="_blank"><img src="<?php echo $baseUrl; ?>/img/twactive.png" title="Follow <?php echo Yii::app()->user->dname?> On Twitter" /></a></div>
						<div class="thumbnails span3 offset5"><a href="<?php echo 'https://www.facebook.com/'. $model->artistsProfiles[0]->fb_link;?>" target="_blank"><img src="<?php echo $baseUrl; ?>/img/fbactive.png" title="Like <?php echo Yii::app()->user->dname?> On Facebook"/></a></div>
				...
				<!-- Sidebar with biography-->
					...
					<div class="span12 mgtop5">
						...
						<p class="color-darkblue "><img src="<?php echo $baseUrl; ?>/img/email.png"/><b class="mgleft5"><?php $mail=Users::model()->findByAttributes(array('id'=>$model->users_id)); echo $mail->email;?></b></p>
							...
							<h4 class="mgleft10">ACHIEVEMENTS</h4>
								<hr class="color-white mgtop-5"/>
							  <ul class="font-segoe">
				...
				<!-- Profile Tabs and rest Informations-->	
					<div class="span8 border-radius10" id="profile-data">
						...
						<h6 class="mgtop-10">
						<?php if(Yii::app()->user->id==$model->users->id){ ?>Likes<?php } else{ echo CHtml::ajaxLink( $label = 'Like', $this->createUrl("site/like",array('uid'=>$model->id,'type'=>'like')), $ajaxOptions=array('dataType'=>'json', 'success'=>'function(html){ jQuery("#likeCount").html(html); }' ), $htmlOptions=array() );}?></h6>
					   </span>
							<!-- Songs-->
							...
							<!--Comments-->
							<span class="span2 box-grey"><a href="#comment" data-toggle="modal" ><img src="<?php echo $baseUrl;?>/img/comment.png"/><h4 class="mgtop5" id="commentCount"><?php echo ($model->artistsProfiles[0]->total_comments)?$model->artistsProfiles[0]->total_comments:'0';?></h4><h6 class="mgtop-10">Comments</h6> </a></span>
							<!--Shares-->
							<span class="span2 box-green"><img src="<?php echo $baseUrl;?>/img/shared.png"/><h4 class="mgtop5" id="shareCount"><?php echo ($model->artistsProfiles[0]->total_share)?$model->artistsProfiles[0]->total_share:'0';?></h4><h6 class="mgtop-10"><?php if(Yii::app()->user->id==$model->users->id){ ?>Share<?php } else{ echo CHtml::ajaxLink( $label = 'Share', $this->createUrl("site/like",array('uid'=>$model->id,'type'=>'share')), $ajaxOptions=array('dataType'=>'json', 'success'=>'function(html){ jQuery("#shareCount").html(html); }' ), $htmlOptions=array() );}?></h6> </span>
							<!--Followers-->
						...
						<?php //CVarDumper::dump(count($model->artistsProfiles[0]->followers),10,1);die;?>
									<?php  if(count($model->artistsProfiles[0]->followers)>='5')
									{ ?>
							...
							<div class="span12">
								<span class="thumbnails">
											<?php if(count($model->artistsProfiles[0]->followers)>0)
												   {
										...
										<!-- List of Songs-->
											<article class="tab-pane active" id="songs"> 
												...
												<?php 
													...
													<?php }} else{
													  ?>
													  <h3><i>You haven't uploaded a song yet!</i></h3>
...
var datastring='pname='+document.getElementById("pname").value;
	...
	<div class="tab-content">
	  ...
	  <tr><td><input type="hidden" name="sid" value="" id="sid"></td></tr>
	   ...
	   <tr><td>
	   <?php 
		$playList	= UsersPlaylist::model()->findAll();
