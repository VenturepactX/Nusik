...
class Albums extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name, date_time, status, login_id', 'required'),
			array('status, login_id', 'numerical', 'integerOnly'=>true),
			array('name', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, date_time, status, login_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			...
			'pAlbumComments' => array(self::HAS_MANY, 'PAlbumComments', 'albums_id'),
			'photoses' => array(self::HAS_MANY, 'Photos', 'albums_id'),
		);
	}
