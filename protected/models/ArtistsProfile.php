...
class ArtistsProfile extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('profile_id, geners_id, biography, achivement, created_date, total_comments, total_likes, total_share, total_followers', 'required'),
			array('profile_id, geners_id, status, total_comments, total_likes, total_share, total_followers', 'numerical', 'integerOnly'=>true),
			array('biography, achivement', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, profile_id, geners_id, biography, achivement, created_date, status, total_comments, total_likes, total_share, total_followers', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'artistsLikes' => array(self::HAS_MANY, 'ArtistsLike', 'artists_profile_id'),
			'profile' => array(self::BELONGS_TO, 'Profile', 'profile_id'),
			'geners' => array(self::BELONGS_TO, 'Genres', 'geners_id'),
			'comments' => array(self::HAS_MANY, 'Comments', 'artists_profile_id'),
			'followers' => array(self::HAS_MANY, 'Follower', 'artists_profile_id'),
			'reportHasProfiles' => array(self::HAS_MANY, 'ReportHasProfile', 'artists_profile_id'),
		);
	}
