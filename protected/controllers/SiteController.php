...
class SiteController extends Controller
{
	...
	public function actionSearch()
	{
		...
		if(isset($_POST['search']))
		{
            ...
            $criteria->join = ' LEFT JOIN `artists_profile` AS `ap` ON t.id = ap.profile_id LEFT JOIN `cities` AS `tu` ON t.cities_id = tu.id Left Join genres as g ON g.id = ap.geners_id';
			$criteria->addCondition("tu.name LIKE '%".$str."%' OR g.name LIKE '%".$str."%'");
			
			
            $finalResult   =    Profile::model()->findAll($criteria);
           //CVarDumper::dump($resultSet,10,1);die;
		/*	--------------
			$n=count($str);
	...
	$this->render('search',array('model'=>$finalResult,'search'=>$_POST['search']));
	}
...
//				CVarDumper::dump($model,10,1);die;
	...
	/* Discover Page */
	public function actionLoadDiscover()
	{
			$modelDiscover = ArtistTrack::model()->findAll(array('select'=>'*, rand() as rand','limit'=>'10','order'=>'rand'));
			$this->renderPartial('loadmore',array('modelDiscover'=>$modelDiscover));
	}
	/* Discover Ajax*/
	public function actionDiscover($type)
	{
			if($type=="liked")
			{
				$modelDiscover=ArtistTrack::model()->findAll(array('order'=>'total_likes DESC'));
			}
			elseif($type=="shared")
			{
				$modelDiscover=ArtistTrack::model()->findAll(array('order'=>'total_shares DESC'));
			}
			else
			{
			$modelDiscover = ArtistTrack::model()->findAll(array('select'=>'*, rand() as rand','limit'=>'10','order'=>'rand'));
			}
			$this->renderPartial('discover',array('modelDiscover'=>$modelDiscover));
